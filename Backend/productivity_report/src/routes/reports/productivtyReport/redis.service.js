...
const setUserAppData = async (key, value) => {
    key = key ? key.trim() : ""
    if (key && value) {
        let setData = await setAsync(key, JSON.stringify(value), 'EX', 60 * 60 * 24);
        return setData ? { code: 201, status: setData === 'OK' } : { code: 400, error: "Not inserted" };
    } else {
        return { code: 404, error: 'Inputs missing' };
    }
};

const getUserAppData = async (key) => {
    key = key ? key.trim() : "";
    if (key) {
        let data = await getAsync(key);
        return data ? { code: 200, data: JSON.parse(data) } : { code: 404, error: 'Data not found', data: null }
    } else {
        return { code: 404, error: 'Inputs missing' };
    }
};
...
module.exports = { setUserAppData, getUserAppData, setUserMetaData, getUserMetaData, getAsync, setAsync, client };
