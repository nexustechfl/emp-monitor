...
CREATE TABLE `employee_dept_email_reports`(
    `id`              bigint(20) UNSIGNED NOT NULL  AUTO_INCREMENT,
...
ADD FOREIGN KEY(`email_report_id`) REFERENCES email_reports(`id`) ON DELETE CASCADE ON UPDATE CASCADE,
ADD FOREIGN KEY(`employee_id`) REFERENCES employees(`id`) ON DELETE CASCADE ON UPDATE CASCADE,
ADD FOREIGN KEY(`department_id`) REFERENCES organization_departments(`id`) ON DELETE CASCADE ON UPDATE CASCADE;
--
...
--
-- Constraints and ADD COLUMN for table `organization_shifts`
--
ALTER TABLE `organization_shifts`
  ADD `notes` varchar(255),
  ADD `location_id` bigint(20) UNSIGNED,
...
--
-- change type photo_path field for table `users`
--
ALTER TABLE `users`
 MODIFY `photo_path` varchar(512);
