...
class AttCalculator {
    constructor({ employee, attendanceData, shift, range, orgTimezone }) {
        ...
        this.range = range;
        this.shift = shift;
        const { id, emp_code, first_name, last_name, departament, location } = employee;
    ...
    calculate() {
        ...
        for (const day of this.range.by('days')) {
            ...
            if (!status) {
                ...
                if (dayData) {
                    this.attendance[dayOfMonth].log.overtime = 'O';
                    this.attendance[dayOfMonth].log.time = this.parseLogTime(dayData);
                    this.counter.plus('O');
                    this.next();
                    continue;
                }
            ...
            if(dayData && this.minWorkingHoursForHalfday) {
                ...
                if (!this.hasCompletedMinHalfdayWorkingHours({ empStart, empEnd })) {
                    this.attendance[dayOfMonth].log.marker = 'ADLH';
                    this.attendance[dayOfMonth].log.time = this.parseLogTime(dayData);
                    this.counter.plus('A');
                    continue;
                }
            ...
            this.attendance[dayOfMonth].log = this.parseOneDayLog(dayData, time, day);
            this.next();
        }
    ...
    parseOneDayLog(AttData, shiftData, day) {
        const log = {
            time: this.parseLogTime(AttData),
            marker: 'P',
        };
        try {
                ...
                if(haveSecondaryMarkers) {
                    if (isHalf) {
                        ...
                        if ( 
                            ...
                            log.loggedInHalf = FIRST_HALFDAY_SHORT_TXT;
                        } else if ( 
                            moment(empStart).isBetween(
                                moment(shiftStart).add(Math.floor(shiftWorkTime / 2), 'hours'),
                                shiftEnd
                            ...
                            log.loggedInHalf = SECOND_HALFDAY_SHORT_TXT;
                        }
                    }
