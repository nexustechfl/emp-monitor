...
module.exports = async ({ orgId: organization_id, empIds: employeesId, depIds: department_ids, date }) => {
    ...
    try {
        let response = await axios.get(`https://service.empmonitor.com/api/v3/hrms/getAttendanceCustom?location_id=0&department_id=0&role_id=0&date=202207&sortOrder=A&skip=0&limit=500&status=1&start_date=${startDate}&end_date=${endDate}&organization_id=${organization_id}`, { headers: { "Authorization": `Bearer ${process.env.JWT_TOKEN}` } });
        temp = response?.data?.data ?? [];
    }
    catch (err) {
        Logger.error(` ================= HRMS Attendance Selected orgId ${organization_id} Error ${err}===================`);
    }
    let data = [];
